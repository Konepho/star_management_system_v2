...
class UpdateStaffRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            'staff_no' => ['required', 'string', 'max:50', Rule::unique('staff', 'staff_no')->ignore($staff->id)],
            ...
            'user_email' => ['nullable', 'email', 'max:255', Rule::unique('users', 'email')->ignore($userId)],
    ...
    public function shouldCreateUserAccount(array $validated): bool
    {
        ...
        return (bool) ($validated['create_login_account'] ?? false)
            || filled($validated['username'] ?? null)
            || $staff->user()->exists();
    }
    ...
    public function withValidator(Validator $validator): void
    {
        ...
        $validator->after(function (Validator $validator) use ($staff): void {
            ...
            if (blank($validated['username'] ?? null)) {
                $validator->errors()->add('username', 'A username is required when managing a login account.');
            }

            if (! $staff->user && blank($validated['password'] ?? null)) {
                $validator->errors()->add('password', 'A password is required when creating a new login account.');
            }
        });
    }
}
